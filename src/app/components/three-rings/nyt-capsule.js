...
NYTCAP.init = function () {
  ...
  // Inserting Ruffle Flash emulator
  NYTCAP.injectRufflePlayer();
...
NYTCAP.injectHeader = function () {
  const hasFixed =
    this.isProjectPage() ||
    this.doesPageHaveFixedEls() ||
    this.hasElBgImage(document.body) ||
    this.shouldUseFixedHead();
  ...
  let capInner = `<div class="nytc-inner ${
    useDarkTheme ? "nytc-dark" : ""
  }" aria-live="polite">`;
  ...
  const canonicalUrl =
    this.capsuleData.publishedUrl || this.capsuleData.canonicalUrl;
  if (canonicalUrl) {
  ...
  capInner +=
    '<li><a href="' +
    this.getReportAProblemURL() +
    '" target="_blank">Report a problem</a></li>';
  capInner += "</ul>";
  capInner +=
    '<button class="nytc-button" id="nytc-close"><span class="nytc-visually-hidden">Hide header</span><i class="icon"></i></button>';
  capInner += "</div>";
  ...
  if (hasFixed) {
    ...
    document.body.className =
      document.body.className.concat(" has-nytc-header");
  ...
  // close button
  document.getElementById("nytc-close").addEventListener(
    "click",
    function (e) {
...
NYTCAP.doesPageHaveFixedEls = function () {
  ...
  for (i = 0; i < els.length; i++) {
    if (
      window.getComputedStyle(els[i], null).getPropertyValue("position") ===
      "fixed"
    ) {
      // in order for this fn to return false, ALL runs of doesElHaveIgnorableIds && doesElHaveIgnorableClasses must be false
      if (
        this.doesElHaveIgnorableIds(els[i].id) === false &&
        this.doesElHaveIgnorableClasses(els[i].className) === false
      ) {
        return true;
      }
    }
...
NYTCAP.isElBgWhite = function (el) {
  ...
  const whiteRe = /rgb\(255\, 255\, 255\)|rgba\(255, 255, 255, (1|\.\d)?\)/gi;
...
NYTCAP.isDarkBgPage = function () {
  ...
  let colors = elCss.backgroundColor
    .split("(")[1]
    .split(")")[0]
    .split(",")
    .map(function (c) {
...
NYTCAP.injectDGsnippet = function () {
  document.addEventListener("DOMContentLoaded", function () {
    ...
    s.setAttribute(
      "src",
      "https://a1.nyt.com/analytics/dg-governance/governance.min.js"
    );
...
NYTCAP.injectRufflePlayer = function () {
  if (NYTCAP.capsuleData.hasFlash) {
    // Using document.write instead of document.head.append make it blocking, which is important to spoof swfobject.js
    document.write(
      `<script src="https://${window.location.hostname}/_flash/ruffle-config.js"></script>`
    );
    document.write(
      `<script src="https://${window.location.hostname}/_flash/ruffle.js"></script>`
    );
  }
};
